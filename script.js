...
const avg=k/8760, lf=(avg/p)*100;
txt('resultMain',`${fmt(lf,2)}%`); txt('resultSub',`Average demand: ${fmt(avg,2)} kW`);
...
txt('detail1',`Resistance used: ${rp} Ω / 1000 ft`); txt('detail2',pct<=3?'Generally within a common 3% target.':'Above a common 3% target. Consider upsizing conductor.');
txt('formulaOut',`${factor} × ${a} × (${ft} ÷ 1000) × ${rp} = ${fmt(drop,2)} V`);}
document.addEventListener('DOMContentLoaded',function(){setYear();
[['calcConcreteBtn',calcConcrete],['calcProfitBtn',calcProfitMargin],['calcLoadFactorBtn',calcLoadFactor],['calcKwhDepositBtn',calcKwhDeposit],['calcVoltageDropBtn',calcVoltageDrop]].forEach(([id,fn])=>{const el=document.getElementById(id);if(el)el.addEventListener('click',fn);});
});
